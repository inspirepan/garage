...
public class S17_30 {
            ...
            for (int j = i + 1; j < nums.length; j++) {
                ...
                while (left < right) {
                    ...
                    if (sumThree == newTarget) {
                        ...
                        left++;
                        right--;
                    } else if (sumThree < newTarget) left++;
                    else right--;
                }
            }
        }
    ...
    public ListNode removeNthFromEnd(ListNode head, int n) {
        ...
        if (removeHead) {
            if (head.next == null) return null;// 注意这里返回空结点是用null，新建结点LeetCode的系统会默认构造成0
        ...
        if (temp.next == null) q.next = null;
        else q.next = temp.next;
        return head;
    }

    /* 20
    又特么忘记判定空集
    用栈的时候，要记住出栈操作前一定要检查下栈是否为空
    ...
    public boolean isValid(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (s.charAt(i) == ')') {
                if (!stack.empty() && (Character) stack.peek() == '(') stack.pop();
                else return false;
            } else if (s.charAt(i) == ']') {
                if (!stack.empty() && (Character) stack.peek() == '[') stack.pop();
                else return false;
    ...
    /* 好吧这个只是看起来简洁，运行时间还不如上面那个，
    不过上面那个太容易出错了，稍微没搞清楚就错了，还容易越界 */
    ...
    /* 21
    这道题我做得还可以，但是和官方的结果相比，步骤多了太多，因为又人为地把两个链表主动进行了划分，导致产生一堆判断句
    Java 的链表如果用等号把两个结点取相等之后，改变一个的next另一个的next也会跟着改变！*/
    public ListNode mergeTwoLists_RAW(ListNode l1, ListNode l2) {
        ...
        /* 这个循环会在q.next==null或者p.next==null时结束，如果在前者结束，
        说明已经把q完全输入到p中了，就是正确的，所以还要处理后者。*/
    ...
    /* 23
    这题学到了分治法，其实也不难，就是先两两分组（奇数就单独一组），然后两两合并，
    比我这种每次读取n个数快多了 */
    ...
    public ListNode reverseKGroup(ListNode head, int k) {
        ...
        if (count < k - 1) return head;
            ...
            /* 这里是将k个链表结点反转，要分情况讨论
            分这么多情况的原因还是我太菜了，翻转要用三个指针，如果k只有4及以下，那会出现一堆空指针 */
            if (k == 2) k_end.next = head;
            else if (k == 3) {
                k_end.next = t;
                t.next = head;
            } else if (k == 4) {
    ...
    public int strStr(String haystack, String needle) {
        ...
        while (i < haystack.length() && j < needle.length()) {
            if (j == -1 || Character.getNumericValue(needle.charAt(j)) == Character.getNumericValue(haystack.charAt(i))) {
                i++;
                j++;
            } else j = nextval[j];
        }
    ...
    public List<Integer> findSubstring_RAW(String s, String[] words) {
        ...
        for (int i = 0; i < s.length() - winLenth + 1; i++) {
            ...
            boolean a = true;
            /* 这一步的实质是比较子串n分段，和模式串所包含的结果是否一致，
            我在循环中又嵌套了一个循环进去，实际上可以再用一个哈希表 */
            for (int k = 0; k < nums; k++) {
                if (!map.containsKey(words[k])) {
                    a = false;
                    break;
    ...
    public List<Integer> findSubstring(String s, String[] words) {
        List<Integer> ans = new ArrayList<>();
        if (s.length() == 0 || words.length == 0 || words[0].length() == 0) return ans;
        int nums = words.length;
        int len = words[0].length();
        ...
        for (int i = 0; i < s.length() - nums * len + 1; i++) {
            ...
            while (j < nums) {
                ...
                if (map1.containsKey(split_word)) {
                    ...
                    if (map2.get(split_word) > map1.get(split_word)) break;// 如果出现次数超过应该出现的次数
                } else break;// 如果不匹配，就下一个子串 i+1
                j++;
            }
            if (j == nums) ans.add(i);
        }
