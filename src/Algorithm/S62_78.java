...
public class S62_78 {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                if (obstacleGrid[i][j] == 1) {
                    obstacleGrid[i][j] = 0;
                    if (j == 0) fireRow = false;
                    if (i == 0) firstColumn = false;
                } else {
                    if ((firstColumn && i == 0) || (fireRow && j == 0)) obstacleGrid[i][j] = 1;
                    else if ((!firstColumn && i == 0) || (!fireRow && j == 0)) obstacleGrid[i][j] = 0;
    ...
    public String addBinary(String a, String b) {
        ...
        if (carry == 1) {
            ...
            StringBuilder sb = new StringBuilder();
            for (int k = 1; k < len_ans; k++) sb.append(ans[k]);
            return sb.toString();
        }
    }
    ...
    /* 73 矩阵置零
    nice啊, 这次自己想出了标准答案,就是用首行首列做记录,
    两个布尔变量用来判断首行首列本身的情况,最后还要单独处理[0][0] */
    public void setZeroes(int[][] matrix) {
        ...
        for (int k = 1; k < n; k++) {
            if (matrix[0][k] == 0) {
                for (int i = 1; i < m; i++) matrix[i][k] = 0;
            } else {
                if (zero_in_row0) matrix[0][k] = 0;
            }
        }
        for (int l = 1; l < m; l++) {
            if (matrix[l][0] == 0) {
                for (int j = 1; j < n; j++) matrix[l][j] = 0;
            } else {
                if (zero_in_column0) matrix[l][0] = 0;
            }
        }
        if (zero_in_column0 || zero_in_row0) matrix[0][0] = 0;
    }
    ...
    public boolean searchMatrix(int[][] matrix, int target) {
        ...
        while (end > begin) {
            ...
            if (matrix[mid / n][mid % n] < target) begin = mid + 1;
            else end = mid;
        }
    ...
    public String minWindow(String s, String t) {
        ...
        for (int i = 0; i < len_t; i++) freq_t[t.charAt(i) - 'A']++;
            ...
            if (len_sub == len_t) {
                ...
                while (k < 58) {
                    if (freq_s[k] < freq_t[k]) {
                        right++;
                        if (right < len_s) freq_s[s.charAt(right) - 'A']++;
                        break;
                    }
                    k++;
                }
                if (k == 58) return s.substring(left, right + 1);
    ...
    public void dfs78(int[] nums, int subset_length, int pos) {
        ...
        for (int i = pos; i < nums.length - subset_length + 1; i++) {
            int mark = 0;
            if (i > pos && nums[i] == nums[i - 1]) mark = 100; // 第90题需要加上这一句
