...
public class S51_60 {
    ...
    /* 49
    首先的想法是哈希表，每一种词都使用一个哈希表
    这道题看出来很多方法完全不知道，比如map可以直接构造一个ArrayList */
    ...
    public void backtrack51(int row, ArrayList<String> board, int border) {
        ...
        for (int column = 0; column < border; column++) {
           if (!isValid51(board, row, column, border)) continue;
            StringBuilder sb = new StringBuilder();
            for (int k = 0; k < border; k++) {
                if (k == column) sb.append('Q');
                else sb.append('.');
            }
    ...
    public boolean isValid51(ArrayList<String> board, int row, int column, int border) {
        ...
        for (int i = 1; i <= row; i++) {
            if (column - i > -1) {
                if (board.get(row - i).charAt(column - i) == 'Q') return false;
            }
            if (column + i < border) {
                if (board.get(row - i).charAt(column + i) == 'Q') return false;
            }
    ...
    public List<Integer> spiralOrder(int[][] matrix) {
        ...
        if (m == 1) {
            ...
            for (int i : matrix[0]) ans.add(i);
            return ans;
        }
        return spiralO(matrix, 0, 0, m - 1, n - 1);
    }

    public List<Integer> spiralO(int[][] matrix, int start_row, int start_column, int opposite_row, int oppposite_column) {
        ArrayList<Integer> part = new ArrayList<>();
        if (start_row == opposite_row) {
            for (int i = start_column; i <= oppposite_column; i++) part.add(matrix[start_row][i]);
            return part;
        }
        if (start_column == oppposite_column) {
            return part;
        }
        for (int i1 = start_column; i1 <= oppposite_column; i1++) part.add(matrix[start_row][i1]);
        for (int i2 = start_row + 1; i2 <= opposite_row; i2++) part.add(matrix[i2][oppposite_column]);
        for (int i3 = oppposite_column - 1; i3 >= start_column; i3--) part.add(matrix[opposite_row][i3]);
        for (int i4 = opposite_row - 1; i4 > start_row; i4--) part.add(matrix[i4][start_column]);
        if (start_row + 1 == opposite_row || start_column + 1 == oppposite_column) return part;
        List<Integer> subpart = spiralO(matrix, start_row + 1, start_column + 1, opposite_row - 1, oppposite_column - 1);
        for (int i : subpart) part.add(i);
        return part;
    }
    ...
    public boolean canJump(int[] nums) {
        ...
        for (int i = 0; i < nums.length - 1; i++) {
            if (nums[i] == 0 && max_ <= i) return false;
    ...
    public int[][] insert(int[][] intervals, int[] newInterval) {
        ...
        for (int i = 0; i < intervals.length; i++) {
            ...
            if (!E_in && intervals[i][0] > nE) {
                ...
                if (i == intervals.length - 1) ans.add(new int[]{StartPoint, nE});
                continue;
            }
            ans.add(intervals[i].clone());
            if (i == intervals.length - 1 && !S_in && !E_in) ans.add(new int[]{nS, nE});
        }
    ...
    public int[][] generateMatrix(int n) {
        ...
        int j = 0, i = 1;
        while (i <= n * n) {
            for (int k = j; k < n - j; k++) ans[j][k] = i++;
            for (int k = j + 1; k < n - j; k++) ans[k][n - j - 1] = i++;
            for (int k = n - j - 2; k >= j; k--) ans[n - j - 1][k] = i++;
            for (int k = n - j - 2; k > j; k--) ans[k][j] = i++;
            j++;
        }
        return ans;
    }
    ...
    public String getPermutation(int n, int k) {
        ...
        for (int i = 0; i < n; i++) resNum.add(i + 1);
        int residue = k - 1;
        for (int i = 0; i < n; i++) {
            if (n - i < 3) {
                if (residue == 0) {
