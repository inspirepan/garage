...
public class S4_16 {
    /*  4
    不会 理不清
    看了下答案，思路是对的，就是不断二分，但是错就错在应该考虑整体的二分，而不是两个数组分别二分
    ...
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        for (int i = 1; i < Math.min(Math.log(m), Math.log(n)); i++) {
            if ((tail1 - head1) % 2 == 0) medium1 = nums1[(tail1 - head1) / 2];
            else medium1 = (nums1[(tail1 - head1) / 2] + nums1[(tail1 - head1) / 2 + 1]) / 2;

            if ((tail2 - head2) % 2 == 0) medium2 = nums2[(tail2 - head2) / 2];
            else medium2 = (nums2[(tail2 - head2) / 2] + nums2[(tail2 - head2) / 2 + 1]) / 2;

            if (medium1 == medium2) return medium1;
    ...
    public String longestPalindrome(String s) {
        ...
        } else if (ans[2] == 2) return s.substring(ans[0] - ans[1], ans[0] + ans[1] + 2);
        else return "";
    }
    ...
    public String convert(String s, int numRows) {
        ...
        if (rest > 0) line_count[0] = set + 1;// 第一行
        else line_count[0] = set;
        if (rest >= numRows) line_count[numRows - 1] = set + 1;// 最后一行
        else line_count[numRows - 1] = set;

        for (int i = 1; i < numRows - 1; i++) {
            // 中间行
            if (rest < i + 1) line_count[i] = set * 2;
            else if (rest >= i + 1 && rest < r - i + 1) line_count[i] = set * 2 + 1;
            else if (rest >= r - i + 1) line_count[i] = set * 2 + 2;
        }
    ...
    public int reverse(int x) throws OutOfMemoryError {
        ...
        while (k != 0) {
            ...
            if (ans > (Integer.MAX_VALUE - p) / 10) return 0;
            ans = 10 * ans + p;
            k = k / 10;
        }
        return (x > 0) ? ans : -ans;
    ...
    public boolean isPalindromeUseString(int x) {
        ...
        if (s.length() % 2 == 0) {
            if (s.charAt(s.length() / 2 - 1) != s.charAt(s.length() / 2)) return false;
    ...
    public String intToRoman(int num) {
        ...
        if (count2 == 9) ans = ans + "CM";
        else if (count2 == 4) ans = ans + "CD";
        ...
        if (count1 == 9) ans = ans + "XC";
        else if (count1 == 4) ans = ans + "XL";
        ...
        int count0 = num % 10;
        if (count0 == 9) ans = ans + "IX";
        else if (count0 == 4) ans = ans + "IV";
    ...
    /* 15
    无论如何都要判断输入是否为空，小伎俩
    还是不行，强行求解细节太多了，很容易弄漏特殊情况，这题缝缝补补修改了太多次，面试遇到就死机了
    算法不行，重写！！！！！！！
    动态规划的意思就是说，循环（一般是while）过程中，根据产生的参数的变化，做出对应的调整 */
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            while (pin1 < pin2) {
                if (nums[i] + nums[pin1] + nums[pin2] == 0) {
                    ...
                    while (pin1 < pin2 && nums[pin1] == nums[pin1 + 1]) pin1++;
                    while (pin1 < pin2 && nums[pin2] == nums[pin2 - 1]) pin2--;
                    pin1++;
                    pin2--;
                } else if (nums[i] + nums[pin1] + nums[pin2] > 0) pin2--;
                else pin1++;
            }
        }
        return ans;
