...
public class S1573 {
    public int numWays(String s) {
        ...
        int k = 0;
        for (int i = 0; i < len; i++) if (s.charAt(i) == '1') index[k++] = i;
        if (k == 0) {
            long l = 0;
            if ((len & 1) == 1) for (int i = 0; i < (len - 2); i++) l = (l + (len - 1) / 2) % mod;
            else for (int i = 0; i < (len - 1); i++) l = (l + (len - 2) / 2) % mod;
            return (int) l;
        }
        if (k % 3 != 0) return 0;
        int t = k / 3;
        ...
        for (int i = 0; i < index[t] - index[t - 1]; i++) m = (m + p) % mod;
        return (int) m;
    }
}
