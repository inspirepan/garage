...
class S4 {
    class Solution {
        ...
        public double findMedianSortedArrays(int[] nums1, int[] nums2) {
            ...
            int n = nums2.length;
            if (((m + n) & 1) == 1) {
                return (double) find(0, 0, (m + n) / 2 + 1);
            } else {
        ...
        int find(int a, int b, int index) {
            ...
            if (a >= nums1.length) return nums2[b + index - 1];
            if (b >= nums2.length) return nums1[a + index - 1];
            if (index == 1) return Math.min(nums1[a], nums2[b]);
