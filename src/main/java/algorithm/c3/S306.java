...
public class S306 {
    ...
    public boolean isAdditiveNumber(String num) {
        ...
        for (int i = 1; i < num.length(); i++) {
            ...
            // 考虑第二个数的0
            if (num.charAt(i) == '0') {
                if (helper(i + 1, first, "0", num)) {
                    return true;
                }
            } else {
                for (int j = i + 1; j < num.length(); j++) {
                    String sec = num.substring(i, j);
                    if (helper(j, add(first, sec), sec, num)) {
                        return true;
                    }
                }
            }
    ...
    boolean helper(int start, String expect, String prev, String num) {
        ...
        if (nextLen + start > num.length()) {
            return false;
        }
        if (!num.substring(start, start + nextLen).equals(expect)) {
            return false;
        }
        if (start + nextLen == num.length()) {
            return true;
        }
