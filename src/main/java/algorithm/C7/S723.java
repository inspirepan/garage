...
public class S723 {
    ...
    private boolean checkSable(int[][] board, int[][] candyToCrush) {
        ...
        for (int[] t : candyToCrush) Arrays.fill(t, 0);
        boolean flag = false;
        // check row
        for (int i = 0; i < m; i++) {
            int p = 0;
            while (p < n) {
                int q = p;
                while (p < n && board[i][p] == board[i][q]) p++;
                // [q,p-1]
                if (p > q + 2 && board[i][q] != 0) {
                    // 如果连续的长度大于3，crush
                    flag = true;
                    int t = q;
                    while (t < p) candyToCrush[i][t++] = 1;
                }
            }
        }
        // check col
        for (int j = 0; j < n; j++) {
            ...
            while (p < m) {
                int q = p;
                while (p < m && board[p][j] == board[q][j]) p++;
                if (p > q + 2 && board[q][j] != 0) {
                    flag = true;
                    int t = q;
                    while (t < p) candyToCrush[t++][j] = 1;
                }
            }
        }
        return flag;
    ...
    private void drop(int[][] board, int[][] candyToCrush) {
        ...
        for (int j = 0; j < n; j++) {
            ...
            int d = m - 1;
            while (b >= 0 && d >= 0) {
                if (candyToCrush[d][j] == 0) board[b--][j] = board[d][j];
                d--;
            }
            while (b >= 0) {
                // 空位补0
